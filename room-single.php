		...
		if(isset($_POST['submit'])){
			if(empty($_POST['email']) OR empty($_POST['full_name']) OR empty($_POST['phone_number']) OR empty($_POST['check_in']) OR empty($_POST['check_out'])){
			  echo "<script>alert('Please fill in all fields')</script>";
				...
				if ($currentDateTimestamp > $checkInTimestamp OR $currentDateTimestamp > $checkOutTimestamp) {
					echo "<script>alert('Enter a valid date')</script>";
				} else {
					if ($checkInTimestamp > $checkOutTimestamp OR $checkInTimestamp == $currentDateTimestamp) {
						...
						if (!isset($_SESSION['id'])){
							...
							echo "<script>alert('Please login first, Redirected to login page.')</script>";
						}
						$user_id = $_SESSION['id'];
	    		...
	    		<div class="col-lg-4">
					<form action="room-single.php?id=<?php echo $id; ?>"  method="POST" class="appointment-form" style="margin-top: -568px;">
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="form-group">
									<input type="submit" name="submit" value="Book Now" class="btn btn-primary py-3 px-4">
								</div>
							</div>
						</div>
				</form>
